...
# VecEnv Wrapper for RL training  中文是 矢量化环境 这个的功能是用来搭建物理引擎和神经网络之间的值的传递的 就是通过space来传递
class VecTask():
    def __init__(self, task, rl_device, clip_observations=5.0, clip_actions=1.0):
        ...
        self.clip_obs = clip_observations  #clip 就是上下限了
...
class VecTaskPython(VecTask):
    ...
    def step(self, actions): #  原来是这里！！！vec
        actions_tensor = torch.clamp(actions, -self.clip_actions, self.clip_actions)

        self.task.step(actions_tensor)  #这里又反过来限制了action   在这里step的！！！   所以说这个真的就是向量的转运中心  不过转运的方式是直接通过把仿真作为task成员return
        # 控制space的区间
        # info可以理解成extras
